...
type
  ICatalogRelationModel = interface(IModel)
...
//    function UpdateWordCatalog(const value:TWordCatalog):_Recordset;
  ...
  TCatalogRelationModel = class(TModel, ICatalogRelationModel)
  private
...
constructor TCatalogRelationModel.Create;
begin
  inherited Create;
end;

function TCatalogRelationModel.QueryWordCatalog:TCustomADODataSet;
const
  sql:string =
'SELECT' + #13#10 +
...
function TCatalogRelationModel.QueryWordCatalogTree:TCustomADODataSet;
const
...
const
  sql:string = 'DELETE FROM CatalogRelation WHERE CatalogID = :CatalogID AND ChildCatalogID = :ChildCatalogID';
begin
  SetExecuteSql(sql);
